...
-(id)initWithString:(NSString*)string Encoding:(NSStringEncoding) encoding error:(NSError**)error
{ 
	if (self = [super init])
	{
        _enc = encoding;
		...
		if ([string length] > 0)
		{
			CFStringEncoding cfenc = CFStringConvertNSStringEncodingToEncoding(_enc);
